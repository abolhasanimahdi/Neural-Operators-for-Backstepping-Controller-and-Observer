...
def train_deeponet_model(x, lam_dataset, k_dataset, samples, nx, layers, iterations=500):
    branch_X = lam_dataset.astype(np.float32)
    ...
    branch_layer_sizes = [nx, layers, layers, layers, layers]
    trunk_layer_sizes = [2, layers, layers, layers, layers]
...
def load_deeponet_model(nx, layers, path="Operator.pth"):
    branch_layer_sizes = [nx, layers, layers, layers, layers]
    trunk_layer_sizes = [2, layers, layers, layers, layers]
    net = dde.nn.DeepONetCartesianProd(layer_sizes_branch=branch_layer_sizes, layer_sizes_trunk=trunk_layer_sizes, activation="tanh", kernel_initializer="Glorot uniform", )
...
def compute_k_hat(lam, model, nx):
    ...
    X_mesh, Y_mesh = np.meshgrid(x, x)
    trunk_X = np.vstack([X_mesh.ravel(), Y_mesh.ravel()]).T.astype(np.float32)
    lambda_test = lam(x).reshape(1, -1).astype(np.float32)
